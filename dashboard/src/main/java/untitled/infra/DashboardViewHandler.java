...
public class DashboardViewHandler {
    ...
    public void whenFlieUploaded_then_CREATE_1(
        @Payload FlieUploaded flieUploaded
    ) {
        try {
            if (!flieUploaded.validate()) return;

            // view 객체 생성
            Dashboard dashboard = new Dashboard();
            // view 객체에 이벤트의 Value 를 set 함
            dashboard.setId(flieUploaded.getId());
            dashboard.setFileName(flieUploaded.getName());
            dashboard.setFileSize(flieUploaded.getSize());
